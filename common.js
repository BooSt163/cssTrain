(function() {
	function Slider(options) {
		var _ = this;
		_.slider_wrap =
			$(options.slider).length != 0 ? $(options.slider) : $('.slider');
		...
		_.item =
			_.slider_wrap.find(options.item).length != 0
				...
				: _.slider_wrap.find('.slide-item');
		tabId = 0;
		slideNow = 1;
		slideCount = $('.slide-line').children().length; //счетчик количества слайдов
		_.itemBuilder = function() {
			for (var i = 1; i <= slideCount; i++) {
				//метод, который берет порядковый номер слайда, и выводит заголовок - ссылку на него
